...
DEBUG = False
STDIN = False
INFILENAME = "input/input.txt"
TESTFILENAME = "input/sample.txt"
for arg in sys.argv:
    if arg == "--notest":
        TEST = False
    if arg == "--debug":
                ...
                for rot_fn, spd2_rot in spd2.items():
                    for spd2k, spd2v in spd2_rot.items():
                        ...
                        if len(intersection) >= threshold - 1:
                            scanner_rel_pos[i][j] = (
                                tuple(a - b for a,
                                      b in zip(spd1_start_point, spd2k)),
                                rot_fn,
                            )
                            foundmatch = True
                            break
...
def part1(lines: List[str], test: bool = False) -> int:
    ...
    for i in range(len(scanner_points)):
        ...
        while offset_rotation_stack:
            ...
            points = maplist(
                lambda p: tuple(
                    b - a for a, b in zip(ROT_INVERSES[rot_fn](offset), p)),
...
if TEST:
    ...
    if not test_lines:
        ...
        if expected is None:
            ...
            print(
                f"{bcolors.FAIL}Result: {test_ans_part1} != {expected}{bcolors.ENDC}")
            assert False
...
def part2(lines: List[str], test: bool = False) -> int:
    ...
    for sp1 in POSES:
        for sp2 in POSES:
            M = abs(sp1[0] - sp2[0]) + \
                abs(sp1[1] - sp2[1]) + abs(sp1[2] - sp2[2])
            if M > max_man:
                max_man = M
...
if TEST:
    ...
    if not test_lines:
        ...
        if expected is None:
            ...
            print(f"{bcolors.FAIL}FAIL{bcolors.ENDC}")
            print(
                f"{bcolors.FAIL}Result: {test_ans_part2} != {expected}{bcolors.ENDC}")
            assert False
